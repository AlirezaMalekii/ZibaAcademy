...
//            Route::apiResource('organizations', 'OrganizationController');
            ...
            Route::get('get-user-notifications/{user_id}/{latest?}', [\App\Http\Controllers\Api\v1\Backend\SuperAdmin\NotificationController::class, 'user_notifications'])->name('get.user.notifications');
            Route::get('notification-mark-as-read/{id}', [\App\Http\Controllers\Api\v1\Backend\SuperAdmin\NotificationController::class, 'mark_as_read'])->name('notification.mark.as.read');
            Route::get('get-all-user-unread-notifications/{user_id}/{latest?}', [\App\Http\Controllers\Api\v1\Backend\SuperAdmin\NotificationController::class, 'get_all_user_unread_notifications'])->name('get.all.user.unread.notification');
            Route::get('mark-all-of-user-unread-notifications-as-read/{user_id}', [\App\Http\Controllers\Api\v1\Backend\SuperAdmin\NotificationController::class, 'mark_all_of_user_unread_notifications_as_read'])->name('mark.all.user.notifications.as.read');
            Route::get('delete-all-user-notifications/{user_id}', [\App\Http\Controllers\Api\v1\Backend\SuperAdmin\NotificationController::class, 'delete_all_user_notifications'])->name('delete.all.user.notifications');
        });
